...
; a subdir (e.g. "wizard" or "maia" within runtime "geopixe").
	...
	gprint,'startupp: starting directory: '+fix_path( file_expand_path('.'))
	geopixe_root = ''
	if file_test('../geopixe', /dir) then begin
		geopixe_root = fix_path( file_expand_path('../geopixe'))
		gprint,'startupp: found normal geopixe path: '+geopixe_root
	endif else if file_test('../../geopixe', /dir) then begin
		geopixe_root = fix_path( file_expand_path('../../geopixe'))
		gprint,'startupp: found geopixe path from bin: '+geopixe_root
	endif else begin
		gprint,'startupp: no "geopixe" directory found ...'
...
;		No "geopixe" runtime dir found (did it get renamed?), so find "GeoPIXE.sav"
		...
		endelse
	endelse
	gprint, 'startupp: GeoPIXE root: ', geopixe_root
